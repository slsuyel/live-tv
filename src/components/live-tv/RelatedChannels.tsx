...
export default function RelatedChannels({
  ...
  return (
    <div className="border-t pt-4 border-slate-150 dark:border-slate-800 space-y-3 transition-colors duration-200">
      <div className="flex justify-between items-center">
        ...
        <span className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">
          Related Streams
        </span>
      </div>
      ...
      <div className="flex gap-2 sm:gap-2.5 overflow-x-auto pb-1.5 no-scrollbar">
        {relatedList.map((related, idx) => (
          <button
            ...
            className="group flex flex-col items-center justify-center p-2.5 bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-slate-800 hover:border-blue-200 dark:hover:border-blue-900/60 hover:bg-blue-50/20 dark:hover:bg-blue-950/10 rounded-xl transition-all duration-200 text-center space-y-1.5 relative overflow-hidden shrink-0 w-24 sm:w-28 cursor-pointer"
          >
            <div className="h-8 w-8 sm:h-9 sm:w-9 bg-white dark:bg-slate-905 border border-slate-100 dark:border-slate-800 rounded-lg p-1 flex items-center justify-center shadow-xs shrink-0 group-hover:scale-105 transition-transform">
              {related.logo ? (
                <img
                  src={related.logo}
                  alt={related.name}
